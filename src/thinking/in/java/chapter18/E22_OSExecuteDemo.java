...
class OSExecute2 {
	public static List<String> command(String command) {
		...
		try {
			...
			BufferedReader results = new BufferedReader(new InputStreamReader(process.getInputStream(), "GB2312"));
			String s;
			while ((s = results.readLine()) != null)
				output.add(s);
			BufferedReader errors = new BufferedReader(new InputStreamReader(process.getErrorStream(), "GB2312"));
...
public class E22_OSExecuteDemo {
	public static void main(String[] args) {
		List<String> result = OSExecute2.command("javap bin/thinking/in/java/chapter18/E22_OSExecuteDemo");
		for (String s : result)
